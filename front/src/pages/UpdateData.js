...
class UpdateData extends React.Component {
    ...
    componentDidMount(){
      axios.get('http://localhost:3001/admin/edit-adp/'+this.state.id).then(res => {
    ...
    handleSubmit = (e) => {
      e.preventDefault();
      axios.post('http://localhost:3001/admin/edit-adp', this.state).then(res => {
        console.log(res.data);
        if(res.data.result){
          alert("Success")
          this.setState({redirect: '/all'});
        }
      }).catch(error => {
        console.log(error);
      });
    ...
    render(){
        ...
        return(
            ...
            <div class="col-lg-13 m-auto">
                <h1 class="h1-white">แก้ไขข้อมูล หมายเลข {this.state.id}</h1>
            </div>
                <div className="row">
                    ...
                    <div class="form-group">
                        <label class="white-font">AdjClose</label>
